...
def check_for_model():
    ...
    for file_path in list(filter(lambda x: x.endswith('_model'), os.listdir(path))):
        if file_path == 'base_model':
            print("found model")
            return True
        else:
            return False
...
for csv_file_path in list(filter(lambda x: x.endswith('.csv'), os.listdir(path))):
    ...
    if basemodelcheck:
        #find latest model
        modelpath = os.getcwd() + "\\data\\model\\base_model"
        regressor = load_model(modelpath)

        #compile latest model
        ...
        #save weights
        modelpath = os.getcwd() + "\\data\\model\\base_model"
        regressor.save_weights(modelpath)
    else:         
        #create model
